...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleJsonInvalido(
        ...
        return buildResponse(HttpStatus.BAD_REQUEST, "INVALID_JSON",
                "Valor invalido en el cuerpo de la peticion. " +
                "Verifica los valores de metodoPago y estado.", request);
    }

    // ── Excepción de dominio: pago no encontrado ──
    @ExceptionHandler(PagoNotFoundException.class)
    public ResponseEntity<ErrorResponse> handlePagoNotFound(
            PagoNotFoundException ex,
            HttpServletRequest request) {

        log.warn("[HANDLER] Pago no encontrado en {}: {}", request.getRequestURI(), ex.getMessage());
