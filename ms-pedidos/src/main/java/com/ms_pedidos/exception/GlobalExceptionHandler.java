...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleJsonInvalido(HttpMessageNotReadableException ex,
        ...
        return buildResponse(HttpStatus.BAD_REQUEST, "INVALID_JSON",
                "Valor invalido en el cuerpo de la peticion", request);
    }

    // ── Excepción de dominio: Pedido no encontrado ──
    @ExceptionHandler(PedidoNotFoundException.class)
    public ResponseEntity<ErrorResponse> handlePedidoNotFound(PedidoNotFoundException ex,
            HttpServletRequest request) {

        log.warn("[HANDLER] Pedido no encontrado en {}: {}", request.getRequestURI(), ex.getMessage());
